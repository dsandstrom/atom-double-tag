describe "DoubleTag", ->
  ...
  # tests end to start syncing
  describe "for an end tag", ->
    describe "with an inline div tag", ->
      beforeEach ->
        editor.setText('<div>test</div>')
      ...
      describe "when cursor is at the front of an end tag", ->
        beforeEach ->
          editor.setCursorBufferPosition([0, 11])
  ...
  # tests start to end with diabled end tag sync active
  describe "for a start tag", ->
    beforeEach ->
  ...
  # tests end to start with diabled end tag sync active
  describe "for an end tag (with disable sync start tag from end tag on)", ->
    beforeEach ->
      atom.config.set('double-tag.disableEditEndTagSync', true)
    ...
    describe "with an inline div tag", ->
      ...
      describe "when cursor is at the back of an end tag", ->
        ...
        describe "and a letter is added", ->
          it "does not copy the letter to the start tag", ->
            editor.insertText('s')

            expect(editor.getText()).toBe '<div>test</divs>'

        describe "and a letter is removed", ->
          it "does not remove the letter from the start tag", ->
            editor.backspace()

            expect(editor.getText()).toBe '<div>test</di>'

        describe "and all letters are removed", ->
          it "does not remove all letters from the start tag", ->
            editor.backspace() for [1..3]

            expect(editor.getText()).toBe '<div>test</>'
      ...
      describe "when cursor is at the front of an end tag", ->
        ...
        describe "and a letter is added", ->
          it "does not copy the letter to the start tag", ->
            ...
            expect(editor.getText()).toBe '<div>test</sdiv>'
    ...
    describe "with an tag with class", ->
      ...
      describe "when cursor is at the end tag", ->
        ...
        describe "and a character added", ->
          it "does not copy the tag text to the start tag", ->
            editor.insertText('v')

            expect(editor.getText()).toBe '<div class="css">test</divv>'

        describe "and a tag deleted", ->
          it "does not delete start tag", ->
            editor.backspace() for [1..3]

            expect(editor.getText()).toBe '<div class="css">test</>'
    ...
    describe "with dashed tag", ->
      ...
      describe "when tag is changed", ->
        it "does not copies the new tag to the end", ->
          editor.setCursorBufferPosition([0, 28])
          editor.backspace() for [1..4]
          editor.insertText('-span')

          expect(editor.getText()).toBe '<dashed-div>test</dashed-span>'
    ...
    describe "with class on second line", ->
      ...
      describe "when letter is removed", ->
        it "does not remove letter form end tag", ->
          editor.setCursorBufferPosition([1, 25])
          editor.backspace()
    ...
    describe "with parenthesis after tag", ->
      ...
      describe "when letter is removed", ->
        it "does not remove letter from end tag", ->
          editor.setCursorBufferPosition([0, 30])
          editor.backspace()

          expect(editor.getText()).toBe '<foo ( class="bar">foobar</fo>'
